...
def fitness_func(x):
    ...
    for i in range(2*n-1):
        ...
        if(right_diagnol[i]>1):
            counter+=right_diagnol[i]-1
            
        diagnol_clashes+= 
    clashes=row_clashes+diagnol_clashes
    # print(row_clashes,diagnol_clashes)
...
def genetic_algorithm(population,fitness_func):
    ...
    while(1):
        ...
        print(fitness_with_child[fitness_without_child.index(max_fitness)],generation)
        printboard(fitness_with_child[fitness_without_child.index(max_fitness)][1])
